...
SRC_ROOT = Path(os.environ.get("OMO_BOOK_SOURCE_ROOT", BOOK_ROOT / "src"))
REPO_URL = "https://github.com/nfbs2000/speaky-oh-my-openagent"
LATEST_REF = "dev"
...
EDITION_REF = os.environ.get("OMO_BOOK_EDITION_REF") or git_output("rev-parse", "HEAD")
EDITION_LABEL = EDITION_REF[:10]
CURRENT_SOURCE_ROOT = "packages/omo-opencode/src"
...
def should_skip_repo_path(path: Path) -> bool:
    ...
    return bool(
        ...
        or path.as_posix().startswith("course/book-omo-ko/")
        or path.as_posix().startswith("packages/shared-skills/upstreams/")
    )
...
def resolve_source(token: str) -> ResolvedSource | None:
    ...
    if normalized.startswith("src/"):
        candidates.append(f"packages/omo-opencode/{normalized}")
    else:
        candidates.extend(
            [
                f"{CURRENT_SOURCE_ROOT}/{normalized}",
                f"packages/omo-opencode/{normalized}",
                f"packages/omo-codex/{normalized}",
                f"packages/shared-skills/{normalized}",
            ]
        )

    if normalized.endswith(".ts"):
        base = normalized.removesuffix(".ts")
        if base.startswith("src/"):
            candidates.append(f"packages/omo-opencode/{base}")
            candidates.append(f"packages/omo-opencode/{base}/index.ts")
        else:
            candidates.append(f"{CURRENT_SOURCE_ROOT}/{base}")
            candidates.append(f"{CURRENT_SOURCE_ROOT}/{base}/index.ts")
...
<aside class="sidebar" aria-label="책 목차">
  <div class="brand">
    <p class="eyebrow">OMO Source Book</p>
    <a class="brand-title" href="{html.escape(relative_href(current, Path("index.html")), quote=True)}">오마오 플러그인 하니스 해부</a>
    <p class="brand-subtitle">책의 관점을 유지하되 현재 fork의 패키지 리팩터링과 실제 소스 참조를 함께 제공합니다.</p>
  </div>
  <nav class="toc-nav">
  ...
  <div class="meta-panel">
    <p>Fork: <a href="{REPO_URL}">nfbs2000/speaky-oh-my-openagent</a></p>
...
<html lang="ko">
  <head>
    ...
    <title>{html.escape(title)} | 오마오 플러그인 하니스 해부</title>
    <meta name="description" content="oh-my-openagent 포크를 현재 소스와 함께 읽는 한국어 온라인 책" />
...
<section class="fork-note">
  <h2>현재 fork 기준 보정</h2>
  <p>이 온라인판은 원고의 관점과 장 구성을 유지하지만, 현재 <code>speaky-oh-my-openagent</code> 포크의 리팩터링을 기준으로 소스 경로를 보정합니다. 원고의 오래된 <code>src/...</code> 참조는 가능한 경우 <code>{CURRENT_SOURCE_ROOT}/...</code>로 해석합니다.</p>
  <p>루트 <code>src/</code>는 더 이상 canonical source tree가 아니며, OpenCode 어댑터는 <code>packages/omo-opencode/src/</code>, Codex Light edition은 <code>packages/omo-codex/</code>, 공유 core는 <code>packages/*-core/</code>에 분리되어 있습니다.</p>
</section>
"""
...
<details class="unresolved-refs">
  ...
  <p>이 경로들은 원고에 있지만 현재 <code>nfbs2000/speaky-oh-my-openagent</code> snapshot에는 없어서 404 링크를 만들지 않았습니다.</p>
...
<article class="cover-page">
  <header class="cover-hero">
    <p class="eyebrow">OMO Source Book</p>
    <h1>오마오 플러그인 하니스 해부</h1>
    <p>오래된 원고의 관점은 유지하되, 현재 <code>speaky-oh-my-openagent</code> 포크의 패키지 리팩터링과 실제 소스 경로를 반영한 온라인판입니다.</p>
  ...
  <section class="edition-panel">
    ...
    <p>원고 source는 이 repo의 <code>course/book-omo-ko/src</code> 아래에 포함되어 있고, 오래된 <code>src/...</code> 경로는 현재 <code>{CURRENT_SOURCE_ROOT}/...</code> 계층으로 보정됩니다.</p>
  </section>
  {fork_correction_note()}
